...
static void open_device(const char *device_name, int &fd)
{ // open the device for reading - fd is a reference
  struct stat st;
...
static void init_device(const char *d_name, int fd, unsigned int d_type, unsigned int d_format, unsigned int d_width, unsigned int d_height)
{ // initialize device
  ...
  {
    fprintf(stderr, "%s support output\n", d_name);
  }
  ...
  {
    fprintf(stderr, "%s support read/write\n", d_name);
  }
  if ((cap.capabilities & V4L2_CAP_STREAMING))
  {
    fprintf(stderr, "%s support streaming\n", d_name);
  }
  if ((cap.capabilities & V4L2_CAP_VIDEO_M2M_MPLANE))
  {
    fprintf(stderr, "%s support m2m mplane\n", d_name);
  }

  if ((cap.capabilities & V4L2_CAP_TIMEPERFRAME))
  {
    fprintf(stderr, "%s support timeperframe\n", d_name);
  }
...
static void init_mmap(const char *device_name, int &fd, enum v4l2_buf_type type, struct buffer **bufs_out, unsigned int *n_bufs)
{ // initialize buffer for device
  ...
  { // loop through all the buffers and assign values and functions
    ...
    bufs[b].start = mmap(nullptr, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, buf.m.offset);
...
static void start_capturing(const char *device_name, int &fd, enum v4l2_buf_type type, unsigned int n_bufs)
{ // start capturing with main v4l2 device
  unsigned int i;
  for (i = 0; i < n_bufs; ++i)
  {
    struct v4l2_buffer buf;
...
static void process_image_thread(void)
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      {
        printf("#");
        fflush(stdout); // getting behind in processing frames - adds latency if this is happening
      }
      ...
      if (force_yuyv == 1)
      {
        for (std::size_t i = 0; i < (std::size_t)buf->bytesused / 4; i++)
        {
      ...
      if (fix_csi == 1)
      { // fix for high cpu usage when using HDMI to CSI adapters
...
{
  if (force_yuyv == 1)
  { // if this is enabled - convert from YUY2 to UYVY for NDI
    yuy2Frame.data = (uint8_t *)p;
    if (!converter.Convert(yuy2Frame, uyvyFrame))
    { // convert the YUY2 frame into a UYVY frame - NDI doesn't accept a YUY2 frame
...
static int read_frame(int &fd, enum v4l2_buf_type type, struct buffer *bufs, unsigned int n_buffs)
{ // this function reads the frame from the video capture device
  auto buf = std::make_unique<v4l2_buffer>();
  // CLEAR(buf);
  buf->type = type;
  buf->memory = V4L2_MEMORY_MMAP;
  if (-1 == xioctl(fd, VIDIOC_DQBUF, buf.get()))
  { // dequeue the buffer - dumps data into the previously set mmap
    switch (errno)
    {
    case EAGAIN:
      return 0;
  ...
  if (NDIlib_send_get_no_connections(pNDI_full_send, 1000))
  { // wait for a NDI receiver to be present before continuing - no need to encode without a client connected
    if (image_threaded == 1)
    {
...
static int mainloop(void)
{
  ...
  while (1)
  { // while loop for querying for new data from video capture device and reading new frames
    for (;;)
    {
      struct timeval tv;
      int r;
...
static const struct option
    long_options[] = {
        ...
        {"yuyv", no_argument, nullptr, 'f'},
        {"uyvy", no_argument, nullptr, 'u'},
        {"nv12", no_argument, nullptr, 'm'},
        {"width", required_argument, nullptr, 'x'},
        {"height", required_argument, nullptr, 'y'},
        {"numerator", required_argument, nullptr, 'n'},
        {"denominator", required_argument, nullptr, 'e'},
        {"threaded", no_argument, nullptr, 'i'},
        {"video", required_argument, nullptr, 'v'},
        {"queue", required_argument, nullptr, 'q'},
        {"fix", no_argument, nullptr, 'c'},
        {0, 0, 0, 0}};
